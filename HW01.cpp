...
double eps_threshold = 0.001;

using namespace std;

bool x_not_converge(long double *x_n, long double *x_n_1, int num_of_poly_basis)
{
    long double eps = 0;
...
bool dx_larger_than_threshold(long double *dx, long double *x_n, int num_of_poly_basis)
{
    long double eps = 0;
    for (int i = 0; i < num_of_poly_basis; i++)
    {
...
int main(int argc, char **argv)
{
    ...
    /* ATA + lambda*I */
    long double ATA_for_Newton[10][10] = {}, ATA_for_Newton_orig[10][10] = {};
    long double ATA[10][10] = {};
    for (int i = 0; i < num_of_poly_basis; i++)
    {
        for (int j = 0; j < num_of_poly_basis; j++)
        {
    ...
    printf("Eq is y = %Lf", x[0]);
    for (int i = 1; i < num_of_poly_basis; i++)
    {
        printf(" + %Lf*x^%d", x[i], i);
    }
    long double y[1000] = {};
    long double Er = 0, Er_rmse = 0;
    for (int i = 0; i < num_of_input_data; i++)
    {
        for (int j = 0; j < num_of_poly_basis; j++)
        {
    ...
    long double x_for_Newton_n[10] = {}, x_for_Newton_n_1[10] = {}, Udx[10] = {}, dx[10] = {};
    int iter = -1;

    /* Find the inverse of ATA_for_Newton */
    long double ATA_inverse[10][10] = {}, op = 0;
    for (int i = 0; i < num_of_poly_basis; i++)
    {
    ...
    for (int i = 0; i < num_of_poly_basis - 1; i++)
        printf("%Lf ", x_for_Newton_n[i]);
    printf("%Lf\n", x_for_Newton_n[num_of_poly_basis - 1]);

    printf("Eq is y = %Lf", x[0]);
    for (int i = 1; i < num_of_poly_basis; i++)
    {
        printf(" + %Lf*x^%d", x_for_Newton_n[i], i);
    }
    ...
    do
    {
        ...
        for (int i = 0; i < num_of_poly_basis; i++)
        {
            for (int j = 0; j < num_of_poly_basis; j++)
            {
                tmp_x[i] += ATA_for_Newton_orig[i][j] * x_for_Newton_n[j];
            }
            tmp_x[i] -= ATb[i];
        }
