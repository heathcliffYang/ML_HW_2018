...
int main(int argc, char const *argv[])
{
    ...
    test_problem.l = 2500;
    struct svm_node **X = (struct svm_node **)malloc(sizeof(struct svm_node *) * 5000 + sizeof(struct svm_node) * 5000 * (5000 + 1));
    if (!X)
    {
        printf("fail to malloc\n");
    }
    ...
    for (j = 0, pStruct = (struct svm_node *)(X + 5000); j < 5000; j++, pStruct += (5000 + 1))
    {
        X[j] = pStruct;
    }
        ...
        for (rbf_w = 0.5; rbf_w < 1.6; rbf_w += 0.5)
        {
            for (parameters.C = 0.4; parameters.C <= 0.4; parameters.C += 0.3)
            {
                for (parameters.gamma = 0.01; parameters.gamma <= 0.01; parameters.gamma += 0.4)
                {
                    ...
                    FILE *sv_file = fopen("SV_pre.txt", "a");
                    int sv_count_start = 0;
                    int sv_count_end = 0;
                    for (int j = 0; j < 5; j++)
                    {
                        fprintf(sv_file, "-1,L_RBF model, %f, %f ,%f ,%f\n", parameters.C, parameters.gamma, linear_w, rbf_w);
                        fprintf(sv_file, "SV of class %d: ", j);
                        sv_count_end += L_RBF->nSV[j];
                        for (int i = sv_count_start; i < sv_count_end; i++)
                        {
                            fprintf(sv_file, "%d ,", L_RBF->sv_indices[i]);
                        }
                        fprintf(sv_file, "\n");
                        sv_count_start += L_RBF->nSV[j];
                    }
                    // char model_file_name[1024];
                    // char C_value[10];
